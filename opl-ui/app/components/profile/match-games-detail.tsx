...
export const MatchGamesDetail: React.FC<MatchGamesDetailProps> = ({ matchId, playerId, players }) => {
  const { data: games, isLoading } = useGames({ match_id: matchId });

  if (isLoading) return <CircularProgress size={20} />;
  if (!games || games.length === 0) {
    return <Typography variant="body2" color="text.secondary">No games recorded.</Typography>;
  }
